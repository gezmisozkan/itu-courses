...
void randomly_order_products(int id){  // randomly order products
    int customer_id = id;  // get customer id
    Customer* customer = &customers[customer_id];  // get customer
...
int main() {
    ...
    int prot_flags = PROT_READ | PROT_WRITE;  // flags for mmap
    int map_flags = MAP_SHARED | MAP_ANONYMOUS;  // flags for mmap
    customers = (Customer*)mmap(NULL, sizeof(Customer) * NUM_OF_CUSTOMERS, prot_flags, map_flags, -1, 0);  // allocate memory for customers
    products = (Product*)mmap(NULL, sizeof(Product) * NUM_OF_PRODUCTS, prot_flags, map_flags, -1, 0);  // allocate memory for products
    if (customers == MAP_FAILED) {  // if mmap fails
        perror("mmap");
        exit(EXIT_FAILURE);
    }
    if (products == MAP_FAILED) {  // if mmap fails
        perror("mmap");
        exit(EXIT_FAILURE);
    }
    ...
    for(int i=0;i<NUM_OF_CUSTOMERS;i++){  // for each customer create a child process
        ...
        if(pid == -1) {  // check if fork failed
            ...
            randomly_order_products(i);  // randomly order products
            exit(0);  // exit the child process, if we don't exit, child process will continue to execute the code below
        }
    }
    ...
    munmap(customers, sizeof(Customer) * NUM_OF_CUSTOMERS);  // deallocate memory for customers
    munmap(products, sizeof(Product) * NUM_OF_PRODUCTS);  // deallocate memory for products
